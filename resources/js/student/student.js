...
//     // ==========================================
//     ['code', 'status', 'batch_id', 'major_id','nationality_id', 'admission_at', 'bacc_2_code', 'entrance_exam', 'exit_exam'].forEach(field => {
...
//     const guardian = payload.guardians[0];

//     const guardNatSelect = document.getElementById('guardian_nationality');
//     if (guardNatSelect && guardian.nationality_id) {
//         guardNatSelect.value = guardian.nationality_id;
//     }
...
// /**
//  * Fetches nationalities data bundle and populates registry select options natively
//  */
// async function loadNationalitiesOptions() {
//     const studentNatSelect = document.getElementById('student_nationality');
//     const guardianNatSelect = document.getElementById('guardian_nationality');

//     if (!studentNatSelect && !guardianNatSelect) return;

//     // Use your custom ApiService wrapper and config path natively
//     const { error, data } = await ApiService.request(CONFIG.API_LOOKUPS.nationalities);
//     if (error) {
//         console.error('Failed fetching nationalities database record array');
//         return;
//     }

//     // Adapt cleanly to either root array envelopes or standard data models
//     const nationalitiesList = data.data || data || [];

//     // Build the selection options string directly
//     let optionsHTML = '<option value="" disabled selected>-- ជ្រើសរើសសញ្ជាតិ --</option>';
//     nationalitiesList.forEach(item => {
//         // Fallback checks to support multilingual 'name_kh', 'name_en', or standard 'name' properties
//         const labelText = item.name_kh || item.name || item.name_en || 'Khmer';
//         optionsHTML += `<option value="${item.id}">${labelText}</option>`;
//     });

//     // Populate both select elements securely
//     if (studentNatSelect) studentNatSelect.innerHTML = optionsHTML;
//     if (guardianNatSelect) guardianNatSelect.innerHTML = optionsHTML;
// }
