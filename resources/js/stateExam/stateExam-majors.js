...
export function addMajorRow(dom, major = '', total = '', time = '') {
    ...
    row.innerHTML = `
        ...
        <select class="major-time-input w-36 px-3 py-2 text-sm bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-white/10 rounded-lg text-neutral-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/40">
            ${timeOptionsHtml(time || TIME_SLOTS[0])}
        </select>
        <input type="number" min="0" placeholder="ចំនួន" value="${total}"
               class="major-total-input w-20 px-3 py-2 text-sm bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-white/10 rounded-lg text-neutral-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/40">
