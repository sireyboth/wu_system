 ...
 * Each row is [major name, total]. The form's student_total field is
 * read-only and always recomputed as the sum of these rows.
 */
...
export function addMajorRow(dom, major = '', total = '') {
    const container = dom.form.querySelector('#majorsContainer');
    if (!container) return;

    const row = document.createElement('div');
    row.className = 'major-row flex items-center gap-2';
    row.innerHTML = `
        <input type="text" placeholder="ជំនាញ (e.g. LAW)" value="${major}"
               class="major-name-input flex-1 px-3 py-2 text-sm bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-white/10 rounded-lg text-neutral-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/40">
        <input type="number" min="0" placeholder="ចំនួន" value="${total}"
               class="major-total-input w-24 px-3 py-2 text-sm bg-neutral-50 dark:bg-neutral-950 border border-neutral-200 dark:border-white/10 rounded-lg text-neutral-900 dark:text-white outline-none focus:ring-2 focus:ring-indigo-500/40">
...
export function loadMajorsRows(dom, majors) {
    ...
    if (Array.isArray(majors) && majors.length > 0) {
        majors.forEach((m) => addMajorRow(dom, m.major ?? '', m.total ?? ''));
    } else {
        addMajorRow(dom);
    }
}

/** Reads the current rows back out as [{major, total}, ...], dropping blank rows. */
