...
export function renderTable(dom, students) {
    // 1. Log the DOM object and the array of all students
    console.log('--- DOM elements object ---', dom);
...
const officialDateFormatted = record.issue_date
    ? new Date(record.issue_date).toLocaleDateString(CONFIG.LOCALE, { day: '2-digit', month: 'short', year: 'numeric' })
    ...
    let statusTextKhmer = student.status?.name_kh || 'Active';
    ...
    // Certificate's own print status (Certificate.status: 'pending' | 'printed'), not the student's academic status.
    const certStatusValue = (record.status || 'pending').toLowerCase();
    let statusCertificateBadge = 'bg-rose-50 text-rose-700 dark:bg-rose-500/10 dark:text-rose-400';
    let statusCertificateText = 'មិនទាន់ព្រីន';
    if (certStatusValue === 'printed') {
        statusCertificateBadge = 'bg-indigo-50 text-indigo-700 dark:bg-indigo-500/10 dark:text-indigo-400';
        statusCertificateText = 'បានព្រីនរួច';
    }
    ...
    return `
        <tr class="block md:table-row bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-white/10 rounded-2xl shadow-sm md:shadow-none md:border-0 md:border-b md:rounded-none overflow-hidden md:overflow-visible">
            ...
            <td class="block md:hidden p-0">
                <div class="flex items-center gap-3 p-4 border-b border-neutral-100 dark:border-white/5">
                    ...
                    <div class="shrink-0 flex flex-col items-end gap-1">
                        <span class="inline-flex items-center px-2 py-1 text-[10px] font-bold rounded-full whitespace-nowrap ${statusBadgeClasses}">
                            ${statusTextKhmer}
                        </span>
                        <span class="inline-flex items-center px-2 py-1 text-[10px] font-bold rounded-full whitespace-nowrap ${statusCertificateBadge}">
                            ${statusCertificateText}
                        </span>
                    </div>
                </div>
                ...
                <div class="grid grid-cols-3 border-t border-neutral-100 dark:border-white/5">
                    <button data-action="print" data-id="${record.id}" class="flex items-center justify-center gap-1.5 py-3 text-xs font-medium text-indigo-600 active:bg-indigo-50 dark:active:bg-indigo-500/10">
                    ...
                    <button data-action="edit" data-id="${record.id}" class="flex items-center justify-center gap-1.5 py-3 text-xs font-medium text-amber-600 border-l border-r border-neutral-100 dark:border-white/5 active:bg-amber-50 dark:active:bg-amber-500/10">
                        <svg class="w-4 h-4 pointer-events-none" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                        </svg>
                        Edit
                    </button>
                    <button data-action="delete" data-id="${record.id}" class="flex items-center justify-center gap-1.5 py-3 text-xs font-medium text-rose-600 active:bg-rose-50 dark:active:bg-rose-500/10">
                        <svg class="w-4 h-4 pointer-events-none" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
            ...
            <td class="hidden md:table-cell px-6 py-4">
                <span class="inline-flex items-center px-2.5 py-1 text-xs font-medium rounded-full ${statusCertificateBadge}">
                    ${statusCertificateText}
                </span>
            </td>
            ...
            <td class="hidden md:table-cell p-6 text-right">
                <div class="flex justify-end gap-1.5">
                    <button data-action="print" data-id="${record.id}" class="p-2 text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-500/10 rounded-xl transition-colors" title="Print Certificate">
                        <svg class="w-4 h-4 pointer-events-none" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M6.72 13.829c-.24.03-.48.062-.72.096m.72-.096a42.415 42.415 0 0110.56 0m-10.56 0L6.34 18m10.94-4.171c.24.03.48.062.72.096m-.72-.096L17.66 18m0 0l.318 2.865a.75.75 0 01-.6.865H6.622a.75.75 0 01-.6-.865L6.34 18m11.32 0h1.093c.836 0 1.534-.64 1.6-1.474l.393-4.917a2.25 2.25 0 00-2.245-2.436H4.992a2.25 2.25 0 00-2.245 2.436l.393 4.917c.066.834.764 1.474 1.6 1.474H6.34m11.32 0H6.34m5.66-13.5v3.66m0 0a2.25 2.25 0 100-4.5v4.5z" />
                        </svg>
                    </button>
                    ...
                    data-action="edit"
                    data-id="${record.id}"
                    class="p-2 text-amber-600 hover:bg-amber-50 dark:hover:bg-amber-500/10 rounded-xl transition-colors" title="Edit student">
                        <svg class="w-4 h-4 pointer-events-none" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/>
                        </svg>
                    </button>
                    <button data-action="delete" data-id="${record.id}" class="p-2 text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-500/10 rounded-xl transition-colors" title="Delete student">
                        <svg class="w-4 h-4 pointer-events-none" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/>
                        </svg>
                    </button>
