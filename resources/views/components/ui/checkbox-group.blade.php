@props([
    ...
    'labelKey' => 'name',
    'inline' => false,
    'choices' => null, // ← for Alpine API: "shifts"
])

@php
    $input_name = str_ends_with($name, '[]') ? $name : $name . '[]';
...
<div {{ $attributes->merge(['class' => 'space-y-2']) }}>
    ...
    {{-- ==================== ALPINE / API DATA ==================== --}}
    @if ($choices)
        <div class="{{ $inline ? 'flex flex-wrap gap-4' : 'space-y-3' }}" x-show="{{ $choices }}.length">
            <template x-for="item in {{ $choices }}" :key="item.{{ $valueKey }}">
                <div class="flex items-center">
                    <label class="inline-flex items-center cursor-pointer select-none">
                        <input type="checkbox" :value="item.{{ $valueKey }}" x-model="form.{{ $model_name }}"
    ...
    @else
        <div class="{{ $inline ? 'flex flex-wrap gap-4' : 'space-y-3' }}">
            @foreach ($options as $option)
                ...
                <div class="flex items-center">
                    <label class="inline-flex items-center cursor-pointer select-none">
                        <input type="checkbox" name="{{ $input_name }}" value="{{ $optValue }}"
                            x-model="form.{{ $model_name }}" x-bind:disabled="mode === 'view'"
                            class="w-4 h-4 rounded border-gray-300 bg-white
                                   text-blue-600
                                   checked:bg-blue-600 checked:border-blue-600
                                   dark:bg-gray-700 dark:border-gray-600
